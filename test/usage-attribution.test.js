...
//
// Magnitudes are the medians of a 12514-object sample of `usage` objects
// recorded in real session transcripts: a turn reads a large cached prefix and
// sends almost no fresh input, which is why counting `input_tokens` alone
// measured close to nothing.
//
// The field names come from those same recordings. So does the reason only the
// flat `cache_creation_input_tokens` appears here: the wire also carries a
// nested `cache_creation` breaking the same quantity down by cache TTL, and
// across 538183 recorded objects the two agree on all but 50. They are the same
// tokens counted once, not two quantities to add.
const USAGE = {
  input_tokens: 2,
