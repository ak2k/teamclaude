...
// shared bucket further past it. Upstream issue #175 measured the coupling at
...
// ---------------------------------------------------------------------------
// The issue's matrix, one test per row.
// ---------------------------------------------------------------------------
...
test('family over cap, shared under: gating stays model-scoped', () => {
  ...
  assert.equal(am._isNearQuota(am.accounts[0], FABLE), true);
  // ...and must not bar anything else. This is the property the max could have
  // broken by leaking the family figure into other models' decisions.
...
// ---------------------------------------------------------------------------
// The consumers that inherit the fix, and the one that deliberately does not.
// ---------------------------------------------------------------------------
...
test('_modelWeeklyExhausted stays family-only and does NOT take the maximum', () => {
  // Different question: "can this account serve this family at all", not "is it
  // near any cap". Folding the shared bucket in would skip accounts for probes
  // they could have served, hardening the stale state a probe exists to escape.
  const am = managerWith({ unified7d: 1.0, unified7dFable: 0.2 });
...
test('the reset still names the governing window, not the bucket that won the max', () => {
  ...
  // only because no caller pairs them; `_bandSnapshot` resolves the bucket once
  // and reads both from it. Pinned here so a later "make them consistent" edit
  ...
  assert.equal(am._governingWeekly(am.accounts[0], FABLE), 1.0, 'value is the maximum');
  assert.equal(am._governingWeeklyReset(am.accounts[0], FABLE), now + 900_000,
    'the reset must stay with the governing window, or pressure divides one bucket\'s'
    + ' headroom by another bucket\'s clock');
});

test('published pressure is about the governing window and is not maxed', () => {
  const now = Date.now();
  const am = managerWith({
    unified7d: 1.0, unified7dReset: now + 100_000,
    unified7dFable: 0.2, unified7dFableReset: now + 900_000,
  });
  const [snap] = am._bandSnapshot(am.accounts, FABLE, now).accounts;
  assert.equal(snap.utilization, 0.2,
    'the band snapshot took the gate\'s maximum, so pressure now mixes two windows');
  assert.equal(snap.resetAt, now + 900_000);
});

// ---------------------------------------------------------------------------
// SONNET. The release note names it and docs/routing.md names it, and until
// this test nothing executed that arm: `unified7dSonnet` is only populated by
// the opt-in usage prober, so every existing fixture leaves it null. A release
// note asserting behaviour no test reaches is a claim without evidence.
// ---------------------------------------------------------------------------
...
// are pinned against each other rather than each against a bucket. Without this
// the routing line and the Models row disagreed in a single render.
// ---------------------------------------------------------------------------

// Drives the REAL renderer against the REAL payload and compares its output to
// `_isAvailable`, because an earlier version of this test recomputed the rule
// itself — `gatingUtilization(...) >= switchThreshold` — and never called
// `renderStatus` at all. It was a THIRD derivation of the rule, written to stop
// a second one, and it passed with the renderer reverted to reading the family
// bucket alone. A test that cannot fail when the fix is reverted is not
// evidence, whatever it is named.
function modelsRowFor(am, name) {
  ...
  // `includes(name)` matched the "TeamClaude status" banner and sliced the
  // header block instead of the account's, so the Models row was never found.
