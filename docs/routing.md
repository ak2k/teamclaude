...
With [expiry-pressure routing](#expiry-pressure-routing) on, **pressure** is consulted before that reset timestamp. The two agree wherever headroom is equal — the soonest-resetting of two equally-spent accounts *is* the highest-pressure one — so this generalises the reset preference rather than replacing it, and they part company only where headroom differs, which is where the timestamp alone was misleading: a nearly-drained account resetting in an hour would otherwise beat one holding far more quota that expires in ten. An exact pressure tie still falls through to the soonest reset.
...
When on, TeamClaude routes each **new** session to the least-loaded eligible account and **pins** it there, so a session keeps hitting the same account and preserves its prompt cache — while different sessions spread across accounts instead of funnelling onto one. Account **priority still wins** (a higher-priority account is never skipped to balance load), and a session whose account becomes exhausted re-routes automatically. Off by default; single-session use is unaffected either way.

"Least-loaded" is measured, not counted: the leading term is the **context each account is currently carrying**, summed over the sessions pinned to it, and the session count sits behind it as a tiebreak rather than being replaced. Sessions are not interchangeable — on a live fleet two accounts holding 2 and 4 sessions drew 0.0300 and 0.0125 of their five-hour window per session, so the count alone reported "nicely balanced" while being wrong by 2.4x. A session that spans two weekly families holds one pin per family, often on different accounts, and each account is charged only the context of the buckets it actually served. While a request is in flight, the pin it is spending keeps counting on its account however long the response streams.

**That last part holds ACROSS families, not within one.** Pins are keyed by weekly bucket, so two concurrent requests on the *same* family share a pin and the second re-pins it: the first account's pin is replaced rather than aged out, and no hold can keep a pin that no longer exists. So a long stream on one family can still read as zero load once a later request on that same family lands elsewhere. Cross-family concurrency — the common shape, an Opus stream and a Fable request together — is counted correctly.

An account nobody has measured yet scores zero, so the term cannot discriminate and the tiebreaks behind it decide exactly as they did before: there is no bootstrap constant and nothing to switch on.
...
Each account gets a **pressure** score for the request's model: headroom in the governing weekly bucket divided by the seconds until that bucket resets. High pressure means ample quota about to be forfeited — spend it first. Because headroom is the numerator, a nearly-drained account is *not* preferred merely because its window rolls soon (reset time alone would steer into it). Fable/Sonnet requests are scored on their own weekly bucket, so the same account can rank differently per model. Pressure is deliberately **not** the maximum the eligibility gate takes: it is headroom over the time until *that* window resets, and mixing buckets would divide one bucket's headroom by another bucket's clock. The gate accounts for the shared bucket instead, and it runs first, so an account over its shared cap never reaches the ranking at all. The 5h bucket is not scored — it stays an availability gate, since its much shorter horizon would otherwise numerically drown the weekly comparison this feature exists to make. `teamclaude status --json` reports each account's computed `pressure` (against the shared weekly), so you can see the ordering the router is working from.

Selection then draws from the **top pressure band**, and how that band is sized depends on whether the fleet has reported a five-hour level.

**With a five-hour signal — capacity sizing.** Accounts are admitted in descending pressure order until their combined five-hour headroom reaches `coverage`, measured in whole accounts (default `1`, i.e. as much absorptive capacity as one untouched account). The question that decides whether a fleet gates is *how many accounts must run in parallel to cover demand*, and a single pressure ratio cannot express it: at an 18x–28x pressure gap `tolerance: 1.5` collapses the band to one member, and a one-member band makes `distributeSessions` a silent no-op. Measured on a live fleet, that was 1090 requests to one account and 0 to two others, with distribution on. No demand forecast is involved — the five-hour level *is* the integral of demand, so an account absorbing more than its share climbs toward its own gate and the rule widens on its own. The error is deliberately asymmetric: under-sizing gates the fleet and forces a cache-destroying migration, while over-sizing spends quota that was not going to expire, which costs nothing while the fleet is demand-limited. An account that has never reported a five-hour level is admitted wherever it sorts and counts **nothing** toward `coverage` — capacity nobody has measured cannot be a reason to stop admitting, and being used is how it becomes known.

**Without one — the tolerance ratio.** This is the cold-start state and the probe-off default, so it is the ordinary path rather than an error path: accounts within `tolerance` (a ratio, ≥ 1) of the best pressure in the top priority tier. `tolerance` is the dial between load spreading and expiry pressure — large values approach pure load-balancing, `1.0` is strict highest-pressure-first. The highest-pressure account always qualifies, so a non-empty candidate set never bands to empty.

Inside the band the usual rules apply unchanged — `distributeSessions` still spreads new sessions by load, priority still wins, and the storm ramp still paces failover. Widening the band for parallel capacity does mean it can admit a member whose pressure is well below the best, so the tiebreak inside the band ranks pressure ahead of the reset timestamp (see [Choosing an account](#choosing-an-account)); otherwise a low-pressure member could be picked purely for having an earlier reset.
...
`routing[]` is the band and pick decisions as a report, one entry per scope: a `shared` entry that is always present, plus one per route. Each entry carries the band variant and why it ran (`sized` with `target`/`achieved`, `banded` with `floor` and a `reason`, or `passthrough`), the `ladder[]` of accounts in the order admission considered them with the running coverage total beside each, an `excluded[]` naming every account removed before the band ran and the reason that removed it, and the `pick` with the term that decided, the runner-up, and any account the winner tied with on every term. `candidates + excluded.length` is the account count, so no account is silently in neither list.

It is a report and never a decision: it is recomputed on each status call and nothing reads it back, so publishing an explanation cannot become a feedback path into routing. It describes a plain request — no advisor model, nothing already tried — because those are properties of a request in flight rather than of the fleet. Absence is variant-shaped throughout: `rank` is null wherever the band computed no order (absent pressure, every row under the ratio rule, every lower-tier row), `cumulative` is null only on a row that admitted nobody, and `target`/`achieved`/`floor` are null under the variants that do not have them rather than zero.

`accounts[].inFlight` is a gauge of the same kind, and reads the same way: how many requests that account is carrying at the instant the payload was built. Nothing accumulates and nothing is reset — `0` means nothing is in flight, not that nobody has looked. It is one of the terms selection ranks on (`_pickSnapshot`), and publishing it is what lets a pick that went somewhere unexpected be explained from the payload rather than guessed at. Two consecutive reads showing `2` are two separate observations that happened to agree, not two requests.

`accounts[].pressureAbsent` names the measurement missing behind a `pressure: null` — `no-utilization` (nobody has reported that weekly window), `no-reset` (the utilization arrived but not the instant it resets) or `utilization-not-finite` (a malformed value). It is non-null exactly when `pressure` is null, because both come from one evaluation rather than two agreeing call sites. The first two clear themselves the next time upstream reports; the third does not, and is the one worth looking at an account over. The feature flag is deliberately absent from this list: pressure is computed whether expiry routing is on or off, so a fleet with the feature off publishes real pressures and `pressureAbsent: null`.
