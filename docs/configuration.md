...
| `expiryRouting` | Optional [expiry-pressure routing](routing.md#expiry-pressure-routing) (off by default). Object: `{ enabled, coverage, tolerance, preempt }` — prefer accounts whose governing weekly quota is ample and resets soonest. `coverage` (default `1`) is how much five-hour absorptive capacity the band must add up to, counted in whole accounts; `tolerance` (default `1.5`, ≥ 1) is the pressure ratio the band falls back to when no account has reported a five-hour level |
